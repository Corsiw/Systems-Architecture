...
int main(int argc, char* argv[]) {
    ...
    // Попробуем заранее создать FIFO (не критично, observer может создать сам)
    struct stat st;
    if (stat(FIFO_PATH, &st) == -1 && errno == ENOENT) {
        // Игнорируем ошибки создания, т.к. observer может создать сам
    ...
    // Закрываем FIFO, если был открыт
    if (fifo_fd != -1) {
        close(fifo_fd);
        fifo_fd = -1;
    }
