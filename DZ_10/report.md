...
* Потоки-сумматоры создаются динамически (`malloc`) и работают в detached-режиме (`pthread_detach`), чтобы не блокировать основной поток.
...
## 3. Пример запуска программы

### 3.1 Компиляция

Используем `gcc` с подключением библиотеки pthread:
